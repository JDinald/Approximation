...
int main()
{
    ...
    cout << "1. Points (x,y) | (-3.2, 10), (-2.1, -2), (0.4, 0), (0.7, -7), (2, 7), (2.5, 0), (2.777, 0) " << endl;
    cout << "2. y = sin(5 * x) * exp(x), x [-2 ; 2], delta x = 0.5" << endl;
    cin >> control;
    cout << "Enter degree:" << endl;
    cin >> degree;
    if (control == 1)
    {
        XY = { {-3.2, 10}, {-2.1, -2}, {0.4, 0}, {0.7, -7}, {2, 7}, {2.5, 0}, {2.777, 0} };
    }
    else
    {
        float x = -2;
    ...
    cout << "Choose approx: " << endl;
    cout << "1. Least squares Approx " << endl;
    cout << "2. Chebyshev Poly Approx " << endl;
    cin >> control;
    cout << "Step:" << endl;
    cin >> step;
    ...
    if (control == 1)
    {
        answ = LeastSquaresPolynomialApprox(XY, degree, step);
        letter = 'A';
    }
    else
    {
        answ = chebyshevPolyAppr(XY, degree, step);
        letter = 'T';
    }
    ...
    cout << endl << "Approx points: " << endl;
	for (auto it = answ.begin(); it < answ.end(); it++)
	{
		cout << "x = " << (*it).x << " " << letter <<"(x,A) = " << (*it).y << endl;
	}
    cout << endl << "Points for plot: " << endl;
    cout << "X: " << endl;
    for (auto it = answ.begin(); it < answ.end(); it++)//
        cout << (*it).x << " "; //
    cout << endl;
